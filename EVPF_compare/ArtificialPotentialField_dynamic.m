function APF =  ArtificialPotentialField_dynamic(x, y, theta, x_goal_path, y_goal_path, globalPathTrackingRange, position_accuracy, obstacle, dynamic,angles,dT, simTimeMax,Success_Num)
...
%     APF.Y = zeros(1,simTimeMax);
    ...
    dynamic.center=[4.23,4.23]-dynamic.velocity*0.01*600;
...
%        if  mod(t,60)==0
%        prev_obstacle=plot(dynamic.obstacle(t,:,1), dynamic.obstacle(t,:,2),'b');hold on;
%        end
       Obstacle_count1 = size(obstacle1,1);
        ...
        nablaU_rep = [0 0];
        for i=1:Obstacle_count1    %机器人每个位置考虑了所有的全局障碍物与未知障碍物
            [obst_idx(i), obst_dist(i)] = dsearchn([obstacle1(i,:,1)' obstacle1(i,:,2)'], [x y]);%找到与机器人最近的障碍物上的那个点的索引 
...
%               AAAA=(APF.eta*(1/APF.Qstar - 1/obst_dist(i)) * 1/obst_dist(i)^2)*([x y] - [obstacle(i,obst_idx(i),1) obstacle(i,obst_idx(i),2)]);
                BBBB=APF.eta*(1/APF.Qstar - 1/obst_dist(i)) * ((norm([x y]-[x_goal y_goal]))^2/obst_dist(i)^2)*([x y] - [obstacle1(i,obst_idx(i),1) obstacle1(i,obst_idx(i),2)])...
                     +2/2*APF.eta*(1/APF.Qstar - 1/obst_dist(i))^2*([x_goal y_goal]-[x y]);
                nablaU_rep = nablaU_rep + BBBB;      %改进的人工势场
            end
        end
        APF.distance(t)=obst_dist(Obstacle_count1)+0.35;
        % Calculate final potential
        nablaU = nablaU_att+nablaU_rep;
        APF.Fa(t,:)=nablaU_att;
        APF.Fa_norm(t)=norm(nablaU_att);
        APF.Fv(t,:)=nablaU_rep;
        APF.Fv_norm(t)=norm(nablaU_rep);
        APF.F(t,:)=nablaU; 
        APF.F_norm(t)=norm(nablaU);
        ...
        APF.Vor(t,:)=dynamic.velocity-v_ref;        
        APF.Vor_norm(t)=norm(dynamic.velocity-v_ref);
        t_i = toc;   %计算引力斥力所用时间
        t_max = max(t_max, t_i);
        t_min = min(t_min, t_i);
