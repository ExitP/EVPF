function SVPF = VirtualPotentialField(xo,dxo,ddxo,O_o,Ro,theta,Success_Num)
...
for t=0:tstep:total_time
    ...
    tic;
    [F_vap, F_vrp,obst_dist] =  ds_virtual_force_VPF(Ro,xo(:,round(t/dt+1)),last_xrp,O_o,K_o,Q_star);
    t_i = toc;
    t_max = max(t_max, t_i);
    t_min = min(t_min, t_i);
    ...
    SVPF.Fa(round(t/dt+1),:)=F_vap;
    SVPF.Fv(round(t/dt+1),:)=F_vrp;
    F_vp = (F_vap+F_vrp);   %势场力=引力+斥力
